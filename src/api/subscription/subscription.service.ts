...
export class SubscriptionService {
  ...
  async create(dto: CreateSubscriptionDto) {
    const vacancy = await this.vacancyRepo.findOne({ where: { id: dto.vacansy_id, is_deleted: false, is_active: true } });
  ...
  async findAll() {
    const data = await this.subscriptionRepo.find({ where: { is_deleted: false, is_active: true }, order: { created_at: 'DESC'} });
    return {
      data,
  ...
  async findAllDeleted() {
    const data = await this.subscriptionRepo.find({ where: { is_deleted: true }, order: { created_at: 'DESC'} });
    return {
      data,
  ...
  async findOne(id: string) {
    const subscription = await this.subscriptionRepo.findOne({
      where: { id, is_deleted: false, is_active: true, vacancy: { is_active: true, is_deleted: false} },
      relations: ['vacancy'],
    });
  ...
  async update(id: string, dto: UpdateSubscriptionDto) {
    ...
    if (dto.vacansy_id) {
      const vacancy = await this.vacancyRepo.findOne({ where: { id: dto.vacansy_id } });
